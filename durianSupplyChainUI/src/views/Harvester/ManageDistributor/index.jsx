...
export default function Marketplace() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await contract.methods
                .addDistributor(harvesterAddress)
    ...
    const handleCheckSubmit = async () => {
        const check = await contract.methods.isDistributor(checkHarvesterAddress).call()
        if (check) {
            toast.success(`${checkHarvesterAddress} is distributor`, {
                position: toast.POSITION.TOP_RIGHT,
                autoClose: 2000,
            })
        } else {
            toast.error(`${checkHarvesterAddress} not a distributor`, {
                position: toast.POSITION.TOP_RIGHT,
                autoClose: 5000,
            })
        }
    ...
    const removeHandleSubmit = async () => {
        try {
            await contract.methods
                .removeDistributor(removeHarvesterAddress)
                .send({ from: sessionStorage.getItem("walletAddress") })
